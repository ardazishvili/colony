...
ColonyApplication<T>::ColonyApplication()
{
  ...
  auto camera = std::make_unique<fig::Camera>(
    glm::vec3(0.0f, -45.0f, 60.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  this->_appEnv = std::make_unique<fig::AppEnvPrivate>(
    std::make_unique<fig::GlfwWindow>(_eventFabric.get(), param), std::move(cameras), lightsCoords);
