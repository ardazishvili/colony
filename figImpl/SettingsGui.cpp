...
void SettingsGui::show() {
  if (_visible) {
    ...
    auto it = std::find_if(config.screenSizes.begin(), config.screenSizes.end(),
                           [config](const auto& p) {
    ...
    if (ImGui::Combo("", &selectedResolution, vector_getter,
                     static_cast<void*>(&resolutions), resolutions.size())) {
