...
void ColonyKeyReleaseEvent::process(fig::Camera* camera, fig::EventManager* eventManager)
{
  ...
  if (_key == GLFW_KEY_X) {
    ...
    if (em->_structureToBuild == nullptr) {
      ...
      auto tankFactory =
        std::make_shared<TankFactory>(em->_textureShader,
                                      em->_linesShader,
                                      em->_astar,
  ...
  if (_key == GLFW_KEY_C) {
    ...
    if (em->_structureToBuild == nullptr) {
      em->_structureToBuildStage = BuildStage::SetAngle;
      auto hq = std::make_shared<Hq>(em->_game,
                                     em,
                                     em->_textureShader,
                                     em->_linesShader,
                                     em->_astar,
  ...
  if (_key == GLFW_KEY_B) {
    ...
    if (em->_structureToBuild == nullptr) {
      em->_structureToBuildStage = BuildStage::SetAngle;
      auto b = std::make_shared<Barrier>(em->_textureShader,
                                         em->_linesShader,
                                         fig::EventManager::unProject(em->_window, em->_view, em->_projection),
                                         em->_terrain,
                                         em->_astar);
  ...
  if (_key == GLFW_KEY_T) {
    ...
    if (em->_structureToBuild == nullptr) {
      em->_structureToBuildStage = BuildStage::SetAngle;
      auto b = std::make_shared<Turbine>(em->_textureShader,
                                         em->_linesShader,
                                         em->_game,
                                         fig::EventManager::unProject(em->_window, em->_view, em->_projection));
      em->_game->addStructure(b);
