...
TankFactory::TankFactory(Shader& shader, glm::vec2 position) :
			     ...
			     Shell::Size shellSize)
{
  ...
  auto tank = ::createTank(game, _shader, glm::vec2(x, y), tankType,
		  					   healthLevel,
							   shellSize);
  auto d = 3.0f;
  auto tankDestination = glm::vec2(x - d * ::cos(glm::radians(_view.angle())),
		  		   y - d * ::sin(glm::radians(_view.angle())));
  tank->startMoving(tankDestination);
}

bool TankFactory::isUnderCursor(const glm::vec2& mousePoint)
{
...
UnitBuilders TankFactory::getUnitBuilders(Game& game)
{
  ...
  addUnitBuilder(game, builders, Tank::Type::Light,
		  		 HealthLevel::High,
				 Shell::Size::Small);
  addUnitBuilder(game, builders, Tank::Type::Medium,
		  		 HealthLevel::High,
				 Shell::Size::Medium);
  addUnitBuilder(game, builders, Tank::Type::Heavy,
		  		 HealthLevel::High,
				 Shell::Size::Big);
  return builders;
}
...
void TankFactory::addUnitBuilder(Game& game,
				 ...
				 Shell::Size shellSize)
{
  std::unique_ptr<AbstractUnitBuilder> builder = std::make_unique<TankBuilder>(_shader, game, *this, type, healthLevel, shellSize);
...
void TankFactory::setPosition(glm::vec2 position)
{
  _view.move(position);
}
