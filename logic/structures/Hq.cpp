...
Hq::Hq(Game* game, ColonyEventManager* eventManager, glm::vec3 position, fig::Terrain* terrain) :
...
StructureBuilders Hq::getStructureBuilders()
{
  ...
  std::unique_ptr<AbstractStructureBuilder> tfBuilder = std::make_unique<TankFactoryBuilder>(_game, _eventManager);
  builders.push_back(std::move(tfBuilder));

  std::unique_ptr<AbstractStructureBuilder> bBuilder = std::make_unique<BarrierBuilder>(_game, _eventManager, _terrain);
