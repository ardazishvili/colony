...
View::View(glm::vec3 position)
    : _shader(*SHADERS_MAP[ShaderType::TEXTURE]), _position(position) {}
...
float View::longitude() const { return _position.x * sqrt(2.0f) / R; }
...
float View::k(float phi) const { return 1.0 / (::cos(phi) * ::sqrt(2.0)); }
...
glm::mat4 View::globeModel() const {
  ...
  model = glm::rotate(model, -latitude() + static_cast<float>(M_PI / 2),
                      glm::vec3(0, 1, 0));
  model = glm::scale(model, glm::vec3(VIEW_SCALE * _objScale));
  return model;
}
